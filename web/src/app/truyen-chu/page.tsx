...
const GENRES = ['Tất cả', 'Hành động', 'Hài hước', 'Kinh dị', 'Lãng mạn', 'Viễn tưởng', 'Tâm lý', 'Tiên hiệp', 'Kiếm hiệp'];
const SORT_OPTIONS = [
...
export default function StoryPage() {
  ...
  const queryParams = new URLSearchParams({ type: 'STORY' });
    ...
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className={cn(
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tight text-on-surface">Truyện Chữ</h1>
            <p className="text-zinc-500 text-sm mt-1">Light Novel, Tiểu thuyết và các tác phẩm văn học đặc sắc.</p>
          </div>
          <button
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-zinc-500 font-bold animate-pulse">Đang tải danh sách truyện...</p>
          </div>
        ) : error ? (
          <div className="py-20 text-center bg-red-500/5 border border-red-500/10 rounded-2xl">
             <p className="text-red-400 font-bold">Đã có lỗi xảy ra khi tải dữ liệu.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
            {data?.data?.map((content: any) => (
              <ContentCard key={content.id} title={content.title} slug={content.slug} imageUrl={content.posterUrl} badge="Story" href={`/truyen-chu/${content.slug}`} />
            ))}

            {(!data?.data || data.data.length === 0) && (
              <div className="col-span-full py-20 text-center bg-surface-container/50 border border-outline/10 border-dashed rounded-2xl">
                <p className="text-on-surface/40 font-bold italic">Chưa có truyện nào trong danh mục này.</p>
              </div>
            )}
          </div>
        )}
