...
const GENRES = ['Tất cả', 'Hành động', 'Hài hước', 'Kinh dị', 'Lãng mạn', 'Viễn tưởng', 'Tâm lý'];
const SORT_OPTIONS = [
...
export default function MoviePage() {
  ...
  const queryParams = new URLSearchParams({ type: 'MOVIE' });
    ...
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className={cn(
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tight text-on-surface">Phim Lẻ</h1>
            <p className="text-zinc-500 text-sm mt-1">Khám phá kho phim lẻ chất lượng cao, đa dạng thể loại.</p>
          </div>
          <button
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-40 gap-4">
            ...
            <p className="text-zinc-500 font-black uppercase tracking-widest text-xs animate-pulse font-['Be_Vietnam_Pro']">Đang tải phim...</p>
          </div>
        ) : error ? (
          <div className="py-20 text-center bg-red-500/5 border border-red-500/10 rounded-3xl space-y-4">
             ...
             <p className="text-zinc-400 font-bold uppercase tracking-tight">Không thể tải danh sách phim</p>
             <button
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
            {data?.data?.map((content: any) => (
              <ContentCard key={content.id} title={content.title} slug={content.slug} imageUrl={content.posterUrl} badge="4K" />
            ))}
            ...
            {(!data?.data || data.data.length === 0) && (
              <div className="col-span-full py-20 text-center bg-surface-container/50 border border-outline/10 border-dashed rounded-3xl">
                <p className="text-on-surface/40 font-bold italic uppercase tracking-widest text-xs">Chưa có phim nào trong danh mục này.</p>
              </div>
            )}
          </div>
        )}
