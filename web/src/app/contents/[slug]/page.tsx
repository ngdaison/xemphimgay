...
export default function ContentDetail() {
  ...
  if (!content) return (
    <div className="min-h-screen bg-background flex items-center justify-center text-on-surface p-10 font-['Be_Vietnam_Pro']">
      <div className="text-center space-y-4">
        <div className="w-20 h-20 border-4 border-zinc-800 border-t-primary-container rounded-full animate-spin mx-auto"></div>
        <p className="text-zinc-500 font-bold uppercase tracking-widest text-[10px] animate-pulse">Đang tải nội dung...</p>
      </div>
    </div>
  );
  ...
  const handleWatch = (item: any) => {
    if (isMovie) {
      router.push(`/watch/${content.slug}/${content.movie.contentId}`);
            ...
            <div className="flex flex-col md:flex-row gap-10 items-end w-full">
              ...
              <div className="flex-1 pb-4">
                ...
                <div className="flex items-center space-x-6 text-on-surface/70 mb-6 font-medium">
                  ...
                  <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" /> 2h 15m</span>
                  <span className="flex items-center gap-1.5"><Languages className="w-4 h-4" /> Vietsub/Lồng tiếng</span>
                </div>
                
                <p className="text-on-surface/70 text-lg max-w-2xl mb-8 leading-relaxed line-clamp-3">
                  {content.description || "Một cuộc hành trình đầy cảm xúc, kết hợp giữa kỹ xảo điện ảnh đỉnh cao và cốt truyện sâu sắc về hy vọng và sự hy sinh."}
                </p>
          ...
          <div className="flex flex-col lg:flex-row gap-12">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-2xl font-bold text-on-surface border-l-4 border-primary-container pl-4 uppercase tracking-tight">Danh sách tập</h3>
                <span className="text-slate-400 text-sm">
                  {mediaItems.length} Tập • {content.releaseStatus || 'Đã hoàn thành'}
                </span>
              </div>
                ...
                {mediaItems.map((item: any, index: number) => (
                  ...
                  >
                    <div className="relative aspect-video rounded-lg overflow-hidden border border-outline/10 mb-2 shadow-sm">
                      ...
                      <div className="absolute bottom-2 right-2 bg-black/70 px-1.5 py-0.5 rounded text-[10px] font-bold text-white">24:12</div>
                    </div>
